...
def fetch_with_retries(
    ...
    """Compatibility helper for yfinance retrieval.

    This repository historically used `fetch_with_retries(ticker, attrs, ...)` to fetch a
    statement DataFrame (balance sheet / income / cashflow) from a list of candidate
    yfinance attributes.

    The updated unconditional-TFT preprocessing script also calls `fetch_with_retries(ticker)`
    expecting a `yfinance.Ticker` object.

    Behavior:
      - attrs is None  -> return `yf.Ticker(ticker)` (with retries).
      - attrs provided -> return `(DataFrame, err)` as before.
    """
    if attrs is None:
        last_err = ""
        for attempt in range(retries + 1):
            try:
                ...
                time.sleep(pause_s)
        raise RuntimeError(f"Failed to create yfinance.Ticker({ticker}): {last_err}")

    # --- legacy path: fetch statement DataFrame from candidate attributes ---
    err = ""
    for attempt in range(retries + 1):
        try:
            t = yf.Ticker(ticker)
...
def merge_and_align_statements(
    bs: Optional[pd.DataFrame],
    inc: Optional[pd.DataFrame],
    cf: Optional[pd.DataFrame],
    freq: str = "X",
) -> pd.DataFrame:
    """Merge raw yfinance statements into a single per-period table.

    Parameters
    ----------
    bs, inc, cf:
        Raw yfinance matrices (rows=items, columns=period end dates). Any of them may be None.
    freq:
        A frequency tag ("A", "Q", or a placeholder). The unconditional dataset builder overwrites
        this column after the merge, so the default "X" is sufficient.

    Returns
    -------
    DataFrame with one row per period end date and extracted numeric columns.
    """
    b = build_period_table_from_statement(bs, freq=freq, extractor_fn=extract_balance_sheet_one_period) if bs is not None else pd.DataFrame()
    i = build_period_table_from_statement(inc, freq=freq, extractor_fn=extract_income_one_period) if inc is not None else pd.DataFrame()
    c = build_period_table_from_statement(cf, freq=freq, extractor_fn=extract_cashflow_one_period) if cf is not None else pd.DataFrame()

    df = merge_statements(b, i, c)
...
class FinancialStatementBuilder:
    """OOP wrapper for the Yahoo Finance data-building pipeline."""

    def run_cli(self) -> None:
        """Dispatch to the original command-line entry point."""
        main()
