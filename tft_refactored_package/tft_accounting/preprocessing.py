...
class PanelFetcher:
    ...
    def fetch_panel(ticker: str, sector: str, freq_mode: str) -> pd.DataFrame:
        ...
        if freq_mode == "MIX":
            # When annual and quarterly rows share the same period end date,
            # always keep the quarterly observation. This avoids artificial
            # jumps such as A(2025-06-30) being inserted between quarterly rows.
            # Priority: Q > A > everything else.
            freq_priority = {"Q": 0, "A": 1}
            df["_freq_prio"] = df["freq"].map(freq_priority).fillna(99).astype(int)
            df = (
                df.sort_values(["date", "_freq_prio"], ascending=[True, True], kind="mergesort")
                .drop_duplicates("date", keep="first")
                .drop(columns=["_freq_prio"])
