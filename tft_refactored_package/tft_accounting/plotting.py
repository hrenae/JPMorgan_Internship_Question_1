...
def plot_one_page(
    ...
    n = len(vars_page)
    nrows, ncols = (5, 2) if n >= 9 else (int(np.ceil(n / 2)), 2)
    ...
    for i, v in enumerate(vars_page):
        ...
        if truth_col and truth_col in df.columns and date is not None:
            ...
            if bool(m_future_truth.any()):
                h = ax.scatter(
                    ...
                    marker="x", s=60, color="black", linewidths=2.0,
        ...
        if th_col and th_col in df.columns and date is not None:
            ...
            if bool(m_th.any()):
                h = ax.scatter(
                    ...
                    marker="D", s=45, color="orange", edgecolor="black",
        ...
        if q10 in df.columns and q50 in df.columns and q90 in df.columns and date is not None:
            ...
            if bool(m_q.any()):
                xq = date.loc[m_q]
                h = ax.scatter(xq, y10.loc[m_q], marker="v", color="red", s=40, alpha=0.70, label="TFT Pred P10")
                legend_cache.setdefault("TFT Pred P10", h)
                h = ax.scatter(xq, y50.loc[m_q], marker="s", color="blue", s=50, edgecolor="black", label="TFT Pred P50")
                legend_cache.setdefault("TFT Pred P50", h)
                h = ax.scatter(xq, y90.loc[m_q], marker="^", color="green", s=40, alpha=0.70, label="TFT Pred P90")
        ...
        ax.set_title(v.name, fontsize=10, fontweight="bold")
...
def save_individual_variable_pngs(
    ...
    """Save each variable panel as a standalone high-resolution PNG."""
    ...
    for vidx, var in enumerate(vars_all, start=1):
        title = f"{ticker} | {sector} | {var.name}"
        fig = plot_one_page(
            df=df,
            vars_page=[var],
            tick_locs=tick_locs,
        ...
        filename = f"{vidx:02d}_{_safe_fs_name(var.name)}.png"
...
def plot_ticker_outputs(
    ...
    """Save both the paginated PDF and standalone PNGs for one ticker."""
    plot_ticker_to_pdf(
        df=df,
        vars_all=vars_all,
        out_pdf=out_pdf,
    ...
    return save_individual_variable_pngs(
        ...
        vars_all=vars_all,
        out_dir=out_png_dir,
        figsize=(figsize[0], max(4.8, figsize[1] / 3.5)),
        max_xticks=max_xticks,
        dpi=png_dpi,
        mode=mode,
        plot_llm_q10_q90=plot_llm_q10_q90,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--png_dpi", type=int, default=300, help="DPI for per-variable PNG exports.")
